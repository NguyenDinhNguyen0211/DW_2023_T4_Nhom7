...
//int dataFileId,
        private void updateStatus( String newStatus,String note) throws SQLException, IOException {
        conn = new GetConnection().getConnection("control");
//        String updateStatusAndErrorSql = "UPDATE data_file SET status = ?,note =? WHERE id = ?";
            String updateStatusAndErrorSql = "UPDATE data_file SET status = ?,note =? WHERE update_at = current_date";
...
//        updateStatement.setLong(3, dataFileId);
        ...
        private void updateStatusConfig(String des) throws SQLException {
        String updateStatusAndErrorSql = "UPDATE data_file_configs SET destination = ? WHERE update_at = current_date";
        try (PreparedStatement updateStatement = conn.prepareStatement(updateStatusAndErrorSql)) {
...
//        updateStatement.setLong(2, dataFileConfigId);
        ...
        public boolean transformDataBankDim() throws IOException, SQLException {
        Connection staging = new GetConnection().getConnection("staging");
        warehouse = new GetConnection().getConnection("warehouse");

        String useDatabaseSQL = "USE warehouse";
        Statement useDatabaseStatement = warehouse.createStatement();
        useDatabaseStatement.execute(useDatabaseSQL);

        // Transform dữ liệu vào bảng bank_dim
        String transformBankDimQuery = "INSERT INTO bank_dim (bank_name, dt_expired) " +
        ...
        public boolean transformDataCurrencyDim() throws IOException, SQLException {
        ...
        // Transform dữ liệu vào bảng bank_dim
        String transformBankDimQuery = "INSERT INTO currency_dim (id_bank, currency_code, currency_name, dt_expired)\n" +
        "SELECT\n" +
        "    bd.id_bank,\n" +
        "    er.currency_code,\n" +
        "    er.currency_name,\n" +
        ...
        "JOIN bank_dim bd ON er.bank_name = bd.bank_name";
...
//            preparedStatement.setInt(1, dfConfigId);
        ...
        return false;
            ...
            public boolean transformDataDateDim() throws IOException {
                ...
                // Transform dữ liệu vào bảng bank_dim
                String transformBankDimQuery = "INSERT INTO warehouse.date_dim (date, day, month, year, hour, minute)\n" +
                        "SELECT distinct COALESCE(STR_TO_DATE(date, '%Y-%m-%d %H:%i:%s'), 'default_date') as date,\n" +
                        ...
                        "    MINUTE(STR_TO_DATE(date, '%Y-%m-%d %H:%i:%s')) as minute\n" +
                        "FROM staging.exchange_rate";
...
//            preparedStatement.setInt(1, dfConfigId);
                    int rowsAffected = preparedStatement.executeUpdate();
                    System.out.println("thanh cong date");
                    return rowsAffected > 0;
                } catch (SQLException e) {
                    System.out.println("Thất bại");
                    new GetConnection().logFile("Transform thất bại");
            ...
            public boolean transformDataExchangeRateFact() throws IOException {
                ...
                // Transform dữ liệu vào bảng bank_dim
                String transformBankDimQuery = "INSERT INTO warehouse.exchange_rate_fact (id_date, id_currency, buy_cash_rate, buy_transfer_rate, sale_rate, dt_expired)\n" +
                        "SELECT dd.id_date, cd.id_currency,\n" +
                        "     CASE WHEN er.buy_cash_rate = '-' THEN 0 ELSE CAST(REPLACE(er.buy_cash_rate, ',', '') AS DECIMAL) END AS buy_cash_rate,\n" +
                        "     CASE WHEN er.buy_transfer_rate = '-' THEN 0 ELSE CAST(REPLACE(er.buy_transfer_rate, ',', '') AS DECIMAL) END AS buy_transfer_rate,\n" +
                        "     CASE WHEN er.sale_rate = '-' THEN 0 ELSE CAST(REPLACE(er.sale_rate, ',', '') AS DECIMAL) END AS sale_rate,\n" +
                        "    '2030-01-01' AS dt_expired \n" +
                        "FROM staging.exchange_rate er\n" +
                        "JOIN warehouse.date_dim dd ON STR_TO_DATE(er.date, '%Y-%m-%d %H:%i:%s') = dd.date\n" +
                        "JOIN warehouse.currency_dim cd ON er.currency_code = cd.currency_code\n" +
...
//            preparedStatement.setInt(1, dfConfigId);
                    int rowsAffected = preparedStatement.executeUpdate();
                    System.out.println("thanh cong fact");
                    return rowsAffected > 0;
                } catch (SQLException e) {
                    e.printStackTrace();
                    System.out.println("Thất bại");
...
//        public boolean transformDataExchangeRateFact() throws IOException, SQLException {
//        Connection stagging = new GetConnection().getConnection("staging");
//        warehouse = new GetConnection().getConnection("warehouse");
//
//        String useDatabaseSQL = "USE warehouse";
...
//
//        // Transform dữ liệu vào bảng bank_dim
//        String transformBankDimQuery = "INSERT INTO exchange_rate_fact (id_date, id_currency, buy_cash_rate, buy_transfer_rate, sale_rate, dt_expired)\n" +
//        "SELECT\n" +
//        "    dd.id_date,\n" +
//        "    cd.id_currency,\n" +
//        "    REPLACE(er.buy_cash_rate,',','') ,\n" +
//        "    REPLACE(er.buy_transfer_rate,',',''),\n" +
//        "    REPLACE(er.sale_rate,',',''),\n" +
//        "    '2030-01-01' as dt_expired \n" +
//        "FROM staging.exchange_rate er\n" +
//        "JOIN date_dim dd ON er.date = dd.date\n" +
...
//        }
//        }
//        public boolean transformDataAggregate() throws IOException, SQLException {
//        Connection stagging = new GetConnection().getConnection("staging");
//        warehouse = new GetConnection().getConnection("warehouse");
//
//        String useDatabaseSQL = "USE warehouse";
...
//
//        // Transform dữ liệu vào bảng bank_dim
//        String transformBankDimQuery = "INSERT INTO exchange_rate_aggregate (date, currency_code, currency_name, bank_name, buy_cash_rate, buy_transfer_rate, sale_rate, create_by, update_by, update_at)\n" +
//        "SELECT\n" +
//        "    dd.date,\n" +
//        "    cd.currency_code,\n" +
//        "    cd.currency_name,\n" +
...
//        }
//        }
//        public boolean transformDataAvg() throws IOException, SQLException {
//        Connection stagging = new GetConnection().getConnection("staging");
//        warehouse = new GetConnection().getConnection("warehouse");
//
//        String useDatabaseSQL = "USE warehouse";
...
//
//        // Transform dữ liệu vào bảng bank_dim
//        String transformBankDimQuery = "INSERT INTO warehouse.avg_rate_aggregate (month_avg, year_avg, currency_code, currency_name, bank_name, avg_buy_cash_rate, avg_buy_transfer_rate, avg_sale_rate, create_by, update_by, update_at)\n" +
//        "SELECT\n" +
...
public boolean transformDataAggregate() throws IOException, SQLException {
    ...
    // Transform dữ liệu vào bảng bank_dim
    String procedureCopy = "exchange_aggregate";
    CallableStatement callableStatement = warehouse.prepareCall("{call " + procedureCopy + "()}");
    callableStatement.execute();

    // 7.5.1 kiểm tra có thành công không
    try (PreparedStatement preparedStatement = warehouse.prepareStatement("call " + procedureCopy + "()")) {
//            preparedStatement.setInt(1, dfConfigId);
        int rowsAffected = preparedStatement.executeUpdate();
        System.out.println("thanh cong");
        return rowsAffected > 0;
        ...
        return false;
            ...
            public boolean transformDataAvg() throws IOException, SQLException {
                ...
                String procedureCopy = "avg_agrregate";
                CallableStatement callableStatement = warehouse.prepareCall("{call " + procedureCopy + "()}");
                callableStatement.execute();
...
//            preparedStatement.setInt(1, dfConfigId);
        ...
        t.updateStatus("P","Process data transform");
        t.updateStatusConfig("W");
        ...
        t.updateStatus("C","Transform data succesfull");
