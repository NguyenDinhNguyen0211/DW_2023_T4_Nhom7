...
//	(data_file.update_at < CURRENT_TIMESTAMP )
	public static ResultSet checkStatus(Connection conn, PreparedStatement pre_control, String status,
		...
		pre_control = conn.prepareStatement("SELECT data_file.id, data_file.name, data_file.row_count,"
				...
				+ "and data_file.update_at < CURRENT_TIMESTAMP ORDER BY data_file.update_at DESC LIMIT 1");
	...
	public static void insertWhToMartAvg(Connection conn_wh, Connection conn_mart, PreparedStatement pre_control,
			Connection conn_control, int numRow, int id) throws SQLException, IOException {

		String selectQuery = "SELECT * FROM avg_rate_aggregate where update_at < CURRENT_TIMESTAMP ORDER BY update_at DESC limit "
				+ numRow;
...
//	            writer.close();
				updateErrSQL(pre_control, conn_control, id, "insert exchange", e);
			}
		}
	}
	...
	public static void insertWhToMartExchange(Connection conn_wh, Connection conn_mart, PreparedStatement pre_control,
		...
		String selectQuery = "SELECT * FROM exchange_rate_aggregate where update_at < CURRENT_TIMESTAMP ORDER BY update_at DESC limit "
				+ numRow;
		try (PreparedStatement selectStatement = conn_wh.prepareStatement(selectQuery);
				ResultSet resultSet = selectStatement.executeQuery()) {
	...
	public static void updateErrConnect(PreparedStatement pre_control, Connection conn_control, int id, String mess) {
		try {
			if (GetConnection2.checkE == 0) {
				String sqlE = "UPDATE data_file SET status='E', data_file.update_at=now() WHERE id=" + id;
	...
	public static void updateErrSQL(PreparedStatement pre_control, Connection conn_control, int id, String mess,
...
//		4. Kiểm tra status = "P" && destination = "M" (Kiểm tra có tiến trình đang chạy không)
			...
			if (re1.next()) {
//			4.1 Thông báo tồn tại tiến trình đang chạy.
...
//			5.2 Đóng kết nối dbControl
					conn_control.close();
					System.exit(0);
				}
//		6. Cập nhật status = "P" ở dòng vừa tìm đươc [Control.data_file_configs &&Control.data_file]
				int id = re2.getInt("id");
				numRow = re2.getInt("row_count");
...
//		8. Kết nối DB warehouse
				...
				updateErrConnect(pre_control, conn_control, id, "warehouse");
...
//		10. Kết nối server DB mart.
				...
				updateErrConnect(pre_control, conn_control, id, "mart");
...
//		insertWhToMartExchange(connection_warehouse, connection_mart)
				insertWhToMartAvg(conn_wh, conn_mart, pre_control, conn_control, numRow, id);
				insertWhToMartExchange(conn_wh, conn_mart, pre_control, conn_control, numRow, id);
...
//				ResultSet re3 = checkStatus(conn_control, pre_control, "P", "W");
//				System.out.println(re3.next());
//				int id2;
//				while (re3.next()) {
//					id2 = re3.getInt("id");
//					System.out.println(id2);

//		18. Update status = "C" && destination = "M" [Control.data_file_configs &&Control.data_file]
				String sqlProcess1 = "UPDATE data_file SET status='C', data_file.update_at=now() WHERE id=" + id;
...
//		18. Thông báo chạy thành công
			...
			String fileName = "D:\\LogERR" + date_err.replaceAll("\\s", "").replace(":", "-") + ".txt";
